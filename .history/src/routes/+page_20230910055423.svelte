	...
	*/
	...
	const updateGames = async () => {
		...
		await fetch("/assets/games.json")
			.then((data) => data.json())
			.then((data) => {
				document.querySelectorAll(".gameCategoryButtons").forEach((gCB) => {
					gCB.innerHTML = "";
					for (let i = 0; i < Object.keys(data).length; i++) {
						let gCBT = document.createElement("button");
						gCBT.innerText = Object.keys(data)[i];
						gCBT.addEventListener("click", () => {
							document.querySelectorAll(".gameCategories").forEach((gC) => {
								gC.innerHTML = "";
								for (let i = 0; i < data[Object.keys(data)[i]].length; i++) {
									gC.appendChild(
										createGameCard(
											data[Object.keys(data)[i]][i].name,
											data[Object.keys(data)[i]][i].image,
											data[Object.keys(data)[i]][i].url
										)
									);
								}
							});
						});
						gCB.appendChild(gCBT);
					}
				});
			});
	}
