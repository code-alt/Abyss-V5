<script lang="ts">
	...
	const createGameCard = (name: string, image: string, url: string) => {
	...
	*/
	...
	const updateGames = async () => {
		...
		await fetch("/assets/games.json")
			.then((data) => data.json())
			.then((data) => {
				// make it so that it precreates the game cards, and display them on and off by buttons, also include an ALL button.
				for (let i = 0; i < Object.keys(data).length; i++) {
					let category = Object.keys(data)[i];
					let categoryButton = document.createElement("button");
					categoryButton.innerText = category;
					categoryButton.addEventListener("click", () => {
						document.querySelectorAll(".gameCategoryButtons button").forEach((button) => {
							button.classList.remove("activeGameCard");
						});
						categoryButton.classList.add("activeGameCard");
						document.querySelectorAll('.gameCategories .abyssCard').forEach((card: Element) => {
							(card as HTMLElement).style.display = 'none';
						});
						document.querySelectorAll(".gameCategories .abyssCard." + category).forEach((card) => {
							https://images.pexels.com/photos/2114014/pexels-photo-2114014.jpeg?cs=srgb&dl=pexels-magda-ehlers-2114014.jpg&fm=jpg.style.display = "inline-block";
						});
					});
			...
			.then((data) => {
				...
				document.querySelectorAll('.abyssFeatGames').forEach((fG) => {
					...
					for (let i = 0; i < data.includeGames.length; i++) {
						fG.appendChild(createGameCard(data.includeGames[i].name, data.includeGames[i].image, data.includeGames[i].url));
					}
				});
			});
	};
	...
	const runService = async (url: string) => {
		...
		if (/^(abyss:)(\/)*[a-z]*/.test(url)) {
			...
			switch (url) {
				case 'newtab':
					(document.getElementById('adrbar') as HTMLInputElement)!.placeholder = ts.getConfig().defaultPlaceholder;
					if (ts.getActiveTab().getTabElement().querySelectorAll('.extFrame')) {
						ts.getActiveTab()
							.getTabElement()
							.querySelectorAll('.extFrame')
							.forEach((frame: HTMLElement) => {
								frame.remove();
							});
					}
					...
					return;
				case 'games':
					(document.getElementById('adrbar') as HTMLInputElement)!.placeholder = 'Abyss Games (abyss://games)';
					if (ts.getActiveTab().getTabElement().querySelectorAll('.extFrame')) {
						ts.getActiveTab()
							.getTabElement()
							.querySelectorAll('.extFrame')
					...
					var extensionsTemplate = document.getElementById('games-temp')!.cloneNode(true) as HTMLDivElement;
					...
					return;
				case 'settings':
					(document.getElementById('adrbar') as HTMLInputElement)!.placeholder = 'Abyss Settings (abyss://settings)';
					if (ts.getActiveTab().getTabElement().querySelectorAll('.extFrame')) {
						ts.getActiveTab()
							.getTabElement()
							.querySelectorAll('.extFrame')
					...
					var settingsTemplate = document.getElementById('settings-temp')!.cloneNode(true) as HTMLDivElement;
					settingsTemplate.id = '';
				...
				default:
					...
					var invalidMainTemplate = document.getElementById('invalid-temp')!.cloneNode(true) as HTMLDivElement;
