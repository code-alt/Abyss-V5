		...
		if (/^(abyss:)(\/)*[a-z]*/.test(url)) {
			...
			switch (url) {
				case 'newtab':
					...
					if (ts.getActiveTab().findFirstIfram)
					ts.getActiveTab().getTabElement().querySelector('.mainStart').style.display = 'initial';
					ts.getActiveTab().getConnectedElement().innerText = 'New Tab';
					return;
				case 'games':
					...
					ts.getActiveTab().getConnectedElement().innerText = 'Games';
					return;
				case 'settings':
					...
					ts.getActiveTab().getConnectedElement().innerText = 'Settings';
					return;
				default:
					if (ts.getActiveTab().getTabElement().querySelectorAll('.extFrame')) {
						ts.getActiveTab()
					...
					ts.getActiveTab().getConnectedElement().innerText = 'Invalid';
					return;
			}
		}
	};
