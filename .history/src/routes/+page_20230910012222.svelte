<script lang="ts">
  ...
  setTimeout(
	...
	onMount(async () => {
		var idleTimeout;
document.getElementById("textContainer").addEventListener("input", () => {
  clearTimeout(idleTimeout);
  var savedInput = document.getElementById("textContainer").value;
  idleTimeout = setTimeout(() => {
    if (document.getElementById("textContainer").value === "") return;
    if (document.getElementById("autocompleteScript"))
      document.getElementById("autocompleteScript").remove();
    if (savedInput === document.getElementById("textContainer").value) {
      var script = document.createElement("script");
      script.id = "autocompleteScript";
      script.src =
        "https://suggestqueries.google.com/complete/search?client=firefox&q=" +
        document.getElementById("ADRB").value +
		...
		await fetch("/assets/news.json").then((data) => data.json()).then((data) => {
			...
			document.querySelectorAll(".abyssFeatGames").forEach((fG) => {
				...
				for (let i = 0; i < data.includeGames.length; i++) {
					let gC = document.getElementById("abyssCardTemplate")?.cloneNode(true) as HTMLElement;
  ...
  await fetch("/assets/themes.json")
    ...
    //   }
		for (var theme = 0; theme < json.length; theme++) {
			...
			console.log("%cDEV:%c Found %c" + themeName + " %c" + themeURL, "font-weight: bold", "font-weight: normal", "font-weight: bold", "font-weight: normal");
	...
	<center style="overflow:auto">
		...
		<div class="settingCards">
			...
			<div class="abyssCard">
				...
				<br />
				<h2>Panic Key URL</h2>
				<input type="text" placeholder="Enter URL (defaults to Google)" />
			</div>
		</div>
	</center>
</div>
