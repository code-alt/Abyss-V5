		...
		if (/^(abyss:)(\/)*[a-z]*/.test(url)) {
			...
			switch (url) {
				...
				case 'games':
					...
					extensionsTemplate.querySelector(".gameCategoryButtons")!.querySelectorAll("button").forEach((btn) => {
						btn.addEventListener("click", () => {
							extensionsTemplate.querySelector(".gameCategories")!.innerHTML = "";
							for (let i = 0; i < gamesJSON[btn.innerText].length; i++) {
								extensionsTemplate.querySelector(".gameCategories")!.appendChild(createGameCard(gamesJSON[btn.innerText][i].name, gamesJSON[btn.innerText][i].image, gamesJSON[btn.innerText][i].url));
							}
						})
					})
